...
def compare(experiment_a: str, experiment_b: str):
    ...
    console.print(Panel(f"[bold cyan]Experiment Comparison Insight[/bold cyan]\n{experiment_a} vs {experiment_b}"))
    ...
    if insight.likely_causes:
        ...
        for cause in insight.likely_causes:
            ...
            console.print(f"  • [{'green' if pct > 60 else 'yellow'}]{cause['cause']}[/] ({pct:.0f}%)")
            console.print(f"    {cause['reasoning']}")

    if insight.recommended_actions:
        console.print("\n[bold]Recommended Actions:[/bold]")
        for action in insight.recommended_actions:
            p_style = {"high": "red", "medium": "yellow", "low": "dim"}.get(action["priority"], "dim")
            console.print(f"  [{p_style}]{action['type']}[/] ({action['priority']}): {action['message']}")
...
def risk(experiment_id: str):
    ...
    console.print(f"\n[bold]Risk Level:[/bold] [{level_colors.get(assessment.risk_level, 'dim')}]{assessment.risk_level.upper()}[/]")

    if assessment.risks:
        console.print("\n[bold]Risk Details:[/bold]")
        for risk in assessment.risks:
            sev_color = {"info": "dim", "warning": "yellow", "critical": "red"}.get(risk["severity"], "dim")
...
@advise.command()
@click.argument("experiment_id")
def baseline(experiment_id: str):
